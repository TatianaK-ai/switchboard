...
not by convention — see `tools/registry.py`. Reads run autonomously. Writes split in
...
- **Admin-approved writes** — password reset, MFA re-enrolment, group membership,
  account unlock. These pause the graph on a LangGraph `interrupt` and land in an
  approval queue that a human IT admin releases. The caller is told they are waiting
  on a person. **The agent cannot approve its own writes, and no prompt can talk it
  into it — the gate is a code path, not an instruction.**
...
| **What can it actually do?** | `directory.lookup` (read) · `directory.verify` (read) · `kb.search` (read) · `ticket.history` (read) · `ticket.create` (**write**, caller-confirmed) · `privileged.request` (**write**, admin-approved, blocks on interrupt) |
...
| **Human-in-the-loop** | Three points. (a) The caller confirms out loud before any ticket is filed. (b) An IT admin approves every privileged write from a queue; the graph is genuinely suspended on an `interrupt` until they do. (c) The post-call review flags calls for human audit — every escalation, every failed verification, and any call the reviewer scores below threshold. |
   ...
   intake ──> verify ──┬── triage ──┬── resolve ──┬── close   │
                  ...
                  (2 failures)      │        (no fix found)   │
                       └────────────┴──> escalate ────────────┘
                                     ...
                                     privileged write?
                                            │
                                    interrupt ──> approval queue ──> human admin
      └──────────────────────────────────────────────────────┘
                                  │
                            call ends
                                  ▼
